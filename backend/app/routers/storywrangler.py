...
async def term_series(
    ...
    entity: Optional[str] = Query(None, description="Global entity ID (e.g. 'wikidata:Q30') or local ID. Omit for datasets without entity_mapping."),
    dates: Optional[str] = Query(None, description="Date range: a single date '2024-06-01' or 'start,end' like '2024-01-01,2024-12-31'. Omit for full history (fast-path types only — types outside the sparkline vocabulary need dates= to bound the scan)."),
...
async def term_series_batch(
    ...
    dates: Optional[str] = Query(None, description="Date range: a single date or 'start,end'. Omit for full history (fast-path types only — out-of-vocabulary types need dates=)."),
