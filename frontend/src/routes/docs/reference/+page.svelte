...
<Table.Root>
	...
	<Table.Body>
		...
		<Table.Row>
			...
			<Table.Cell>string</Table.Cell>
			<Table.Cell>✓</Table.Cell>
			<Table.Cell><code>parquet</code> · <code>parquet_hive</code> · <code>duckdb</code> · <code>ducklake</code></Table.Cell>
		</Table.Row>
		<Table.Row>
			...
			<Table.Cell>
				...
				<code>tables_metadata</code> (logical table name → file path list),
				<code>ducklake_data_path</code> (ducklake only — path to the catalog <code>.duckdb</code> file),
				<code>partitioning</code> (partition scheme, e.g. <code>{'{keys: ["date", "country"]}'}</code>),
				<code>availability</code> (coverage ranges per entity).
			</Table.Cell>
		</Table.Row>
		<Table.Row>
			...
			<Table.Cell></Table.Cell>
			<Table.Cell>
				Schema declaration for entity ID resolution: <code>{'{local_id_column}'}</code> — the
				dataset column that holds local entity identifiers.
		...
		<Table.Row>
			<Table.Cell><code>endpoint_schema</code></Table.Cell>
			<Table.Cell>object</Table.Cell>
			<Table.Cell></Table.Cell>
			<Table.Cell>Query contract for this dataset (instrument compatibility). See below.</Table.Cell>
		</Table.Row>
		<Table.Row>
		...
		<Table.Row>
			...
			<Table.Cell></Table.Cell>
			<Table.Cell>
				<code>sources</code> (external raw data URLs keyed by dimension),
				<code>derived_from</code> (<code>["domain/dataset_id"]</code> — upstream registered datasets),
...
<p>
	Each <code>entity_id</code> value uses a <code>namespace:identifier</code> format. The
	namespace (e.g. <code>wikidata</code>, <code>orcid</code>) is the <strong>identifier
	system</strong> — not a semantic category. The entity type is auto-derived from the prefix at
	registration time. The SDK validates each <code>entity_id</code> format against the
...
<h2>Endpoint schema</h2>

<p>
	The <code>endpoint_schema</code> field declares what query types a dataset supports — the contract
