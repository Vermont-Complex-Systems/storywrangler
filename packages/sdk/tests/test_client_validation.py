...
DatasetClient.top_ngrams validates dates against cached registry metadata
before any request goes out: dateless datasets reject dates entirely,
mongodb pass-through datasets reject ranges. Metadata is stubbed; the
failure paths never touch the network.
"""
import pytest
...
class TestValidateDates:
    def test_dateless_dataset_rejects_dates(self):
        client = make_client({"transform": {"filter_dimensions": ["town"]},
                              "data_format": "parquet"})
    ...
    def test_omitting_dates_skips_validation(self):
        ...
        # the request itself would go to the network, so only check that
        # _validate_dates passes.
        client = make_client({"transform": {"filter_dimensions": ["town"]},
                              "data_format": "parquet"})
        client._validate_dates(None, None)  # no raise

    def test_range_ok_for_parquet(self):
        client = make_client({"transform": {"time_dimension": "date"},
